...
def get_gutenberg_pieces(desired_lengths):
    ...
    for path_to_text, desired_length in zip(gutenberg_texts.glob("*"), desired_lengths):
        PGID = path_to_text.name
        text = get_gutenberg_piece(path_to_text, desired_length)
        if text is not None:
            texts[PGID] = text
...
def main():
    ...
    parser.add_argument("--input_pickled", type=str, default="")
    ...
    import pandas as pd
    # load PG data ###############
    prompts_dict = get_gutenberg_pieces([300]*100)
    ################
